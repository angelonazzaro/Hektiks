...
<div class="user-general-info">
    <div class="user-card">
        <div class="user-card-header">
            <img src="<%= profile_pic %>?time=<%=System.currentTimeMillis()%>" alt="<%= utente.getUsername() %> immagine profilo">
        </div>
        <div class="user-card-body">
            <p class="hs-4" style="font-weight: bold; color: white; margin-bottom: 0.5rem"><%= utente.getUsername() %></p>
            <p class="text" style="color: var(--grey)">Membro dal: <%= new SimpleDateFormat("dd MMMMMMMMMM yyyy ").format(utente.getData_registrazione()) %></p>
        </div>
    </div>
</div>
<div class="settings-wrapper dashboard-container">
    <h1 class="hs-3">Form di modifica: <%= utente.getUsername() %></h1>
    <form action="<%= request.getContextPath() %>/admin" method="POST" id="form-settings" class="settings-container">
        <div class="form-body">
            ...
            <div class="row">
                ...
                <input type="email" class="form-control" name="email" value="<%= utente.getEmail() %>" placeholder="Email">
            </div>
            <div class="row">
